...
def check_db_exists(): # при инициализации класа
...
table_mappings = {'campaigns' : Campaigns} # для маппинга имен таблиц с экземлярами sqlalchemy.Table
...
def __is_valid(table_name: str, column_values = None) -> bool:
    ...
    if column_values != None: # None - когда выбираем в методе fetchall все поля из таблицы БД

        # проверка на существование полей таблицы БД
        table_instance = table_mappings[table_name]
        columns_from_table = set(table_instance.metadata.tables[table_name].columns.keys())
        ...
        not_valid_columns = columns_from_query - columns_from_table # разность множеств для поиска несуществующих полей
        if len(not_valid_columns) != 0:
            print("Одно или несколько полей, переданных в аргументах, не соответствуют полям из БД")
            return False
        ...
        if isinstance(column_values, Dict):
            ...
            for key, value in column_values.items():
                if not isinstance(value, field_types[key]):
                    print(f"Поле '{key}' из запроса не соответсвует типу {field_types[key]}")
                    return False
...
def fetchall(table_name: str, columns: List = None):
    ...
    if __is_valid(table_name, columns):
        ...
        if columns == None: # для выбора всех полей из таблицы
            ...
            fields = [metadata.table_instance[table_name].columns.get(column) for column in  columns]
            query = select(fields)
        lines = connection.execute(query).fetchall()
        connection.close()
        return lines
    else:
        print("Аргументы, переданные в функцию получения данных из БД, не прошли валидацию")
...
def insert(table_name: str, column_values: Dict):
    ...
    if __is_valid(table_name, column_values):
        table_instance = table_mappings[table_name]
        query = table_instance.insert().prefix_with("OR REPLACE").values(column_values) # "OR REPLACE" - на случай, когда поступила запись с существующим в БД PK
